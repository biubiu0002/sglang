...
//! Integration tests for the `/v1/completions` passthrough route.
...
async fn send_completion(app: axum::Router, body: Value) -> StatusCode {
...
#[tokio::test]
async fn completions_body_forwarded_unchanged_no_input_ids() {
...
#[tokio::test]
async fn completions_prompt_tokens_feed_cache_aware_route_history() {
    ...
    let body = json!({
        "model": "tiny",
        "prompt": "repeatable completions prefix",
        "max_tokens": 16,
        "stream": false,
    });
    assert_eq!(
        send_completion(app.clone(), body.clone()).await,
        StatusCode::OK
    );
    {
    ...
    assert_eq!(send_completion(app, body).await, StatusCode::OK);
    let first_seen = first.captured.lock().unwrap().last_body.is_some();
    let second_seen = second.captured.lock().unwrap().last_body.is_some();
    assert!(
        first_seen && !second_seen,
